...
const config = {
  targetPath: 'H:/data/cyclone/json_format/msc/CENS/',//__dirname + '/ecJSON/',
  logFile: './logs/CENS_Sresolve_log.txt',
  demoFile: "H:/data/cyclone/msc/2014/20140101/z_tigge_c_kwbc_20140101000000_CENS_glob_prod_esttr_glo.xml",
  demoFile2: 'H:/data/cyclone/msc/2014/20140101/z_tigge_c_kwbc_20140101000000_CMC_glob_prod_sttr_glo.xml',
}
...
function combineTC(data){
  let mixData = data.memberList.reduce((final,member)=>{
    let memberNumber = member.ensembleNumber;
    for(let tc of member.TClist){
      ...
      if(found){//找到则把路径插入到相同TC中
        ...
        if(!isNaN(member.ensembleNumber)) newTC.ensembleNumber = member.ensembleNumber;//第几个集合预报
        ...
        if(!isNaN(member.ensembleNumber)) newTC.tracks[0].ensembleNumber = member.ensembleNumber;//第几个集合预报
