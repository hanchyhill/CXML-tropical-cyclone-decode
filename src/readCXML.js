...
const main = async function(){
  let fileURI = path.resolve(__dirname,'./xml/z_tigge_c_kwbc_20190212000000_GEFS_glob_prod_esttr_glo.xml');
  const xmlFile = await fs.readFile(fileURI);
  let result = await parseString(xmlFile);

  fs.writeFile(path.resolve(__dirname,'./xml/20190212000000_GEFS.json'),JSON.stringify(result,null,2));
  ...
  await fs.writeFile(__dirname+'/xml/20190212000000_GEFS_final.json', JSON.stringify(transferData,null,2));
  //return transferData;
